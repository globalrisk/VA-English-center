-- Remove Quizlet embed support: backfill legacy rows and require built-in games only.

UPDATE public.lessons
SET
  builtin_game = COALESCE(builtin_game, 'flashcards'),
  embed_url = NULL
WHERE lesson_type = 'game'
  AND (
    embed_url IS NOT NULL AND trim(embed_url) <> ''
    OR builtin_game IS NULL
  );
